...
Public sheetOpenned As Boolean
...
'Deletes the Pool Data Worksheet if it already exists----------------------
   ...
   'Creates the new Worksheet Pool_Data in which all pool data will be added----------
    ...
    Do Until sheetNumber > Worksheets.Count - 2
        ...
        placeKeeper = 1
        keeper = 3
        totSamples = Worksheets(sheetNumber).Application.WorksheetFunction.CountIf(myRange, "*?") + Count(myRange)
...
'THIS BUTTON IS NO LONGER IN USE. Command Button 4 takes care of this operation for all pools

  'This button deletes all white samples (samples which are not to be included in the pool) from an individual pool.
    Dim numSamples As Integer
    Dim myRange As Range
    Dim keeper As Integer
    Dim placeKeeper As Integer
    ...
    placeKeeper = 1
    keeper = 3
    Set myRange = Range("B3:B400")
    numSamples = Application.WorksheetFunction.CountA(myRange)
    
    'Deletes rows that are not to be included in the pool----------------------------------
    Do Until placeKeeper > numSamples
        If Range("I" & keeper) = "" Then
            Rows(keeper).Delete
            keeper = keeper
...
If sheetOpenned = False Then
    If Sheet8.Name <> "Pool 1" Or Worksheets.Count > 8 Then
        Exit Sub
    End If
    Dim numSamples As Integer
    ...
    currentName = Sheet8.Name

    
    poolName = InputBox("What is the name of the new pool?")
    If poolName = "" Then
        Sheet8.Name = currentName
    Else
        Sheet8.Name = poolName
    End If
    sheetOpenned = True
End If
